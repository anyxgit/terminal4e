...
final class TerminalChildProcessMonitor {
	...
	private volatile Boolean lastKnownState;
	private volatile long lastInactiveRefreshAt;
	private volatile ScheduledFuture<?> pendingActiveRefresh;

	TerminalChildProcessMonitor(long rootPid, ShellDescriptor shell, Consumer<Boolean> onStateChanged) {
	...
	private void refreshNow() {
		...
		boolean hasChildren = inspection.hasMeaningfulProcesses();
		Boolean previous = lastKnownState;
		if (previous == null || previous.booleanValue() != hasChildren) {
			lastKnownState = Boolean.valueOf(hasChildren);
			printDiagnostics(inspection);
			onStateChanged.accept(Boolean.valueOf(hasChildren));
		}
	}
	...
	private static final class ProcessTreeInspector {
		...
		private static List<ProcessEntry> listWindowsProcesses() {
			List<String> lines = runCommand(Arrays.asList("wmic", "process", "get", "ProcessId,ParentProcessId,Name", "/FORMAT:CSV"), 5);
			...
			List<String> psLines = runCommand(Arrays.asList(
					...
					"-NoProfile",
					"-Command",
					"Get-CimInstance Win32_Process | Select-Object Name,ParentProcessId,ProcessId | ConvertTo-Csv -NoTypeInformation"), 6);
		...
		private static List<ProcessEntry> parseWindowsCsv(List<String> lines) {
			...
			for (String rawLine : lines) {
				...
				if (pidIndex.intValue() >= cols.size() || ppidIndex.intValue() >= cols.size() || nameIndex.intValue() >= cols.size()) {
					continue;
				}
	...
	private static final class WslContext {
		...
		private boolean isEnabled() {
			return command != null && !command.trim().isEmpty() && distribution != null && !distribution.trim().isEmpty();
		}
